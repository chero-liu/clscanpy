...
from clscanpy.tools.preprocessing.highly_variable_genes import variable_filter
from clscanpy.tools.embedding._batch import batch_correction
from clscanpy.tools.preprocessing.graph import neighbors, clustering, find_elbow
from clscanpy.tools.preprocessing.utils import transform_cluster_labels, umap_argue
from clscanpy.tools.embedding.embedding import run_umap, run_tsne
from clscanpy.tools.preprocessing.basicpreprocessing import normalize
...
class Cluster:
    ...
    def run(self):
        ...
        if self.batch_name != "batchid" and self.batch_method != "No":
            self.adata.obs["batchid"] = pd.Categorical(
                self.adata.obs[self.batch_name].cat.codes + 1
            )
    ...
    def _process_custom_hvgs(self):
        ...
        if self.hvg_replace:
            ...
            LOGGER.info(
                f"Replaced highly variable genes with {len(matched_genes)} genes from {self.hvg_replace}"
            )
        ...
        if self.hvg_add:
            ...
            matched_genes = match_genes(target_genes)
            # 添加基因到现有高变基因集合
            current_hvgs = self.adata.var_names[
            ...
            LOGGER.info(
                f"Added {len(matched_genes)} genes to highly variable genes from {self.hvg_add}"
            )
        ...
        if self.hvg_remove:
            ...
            remaining_hvgs = [
                gene for gene in current_hvgs if gene not in matched_genes
            ]
            self.adata.var["highly_variable"] = self.adata.var_names.isin(
                remaining_hvgs
            )
            LOGGER.info(
                f"Removed {len(matched_genes)} genes from highly variable genes using {self.hvg_remove}"
            )
