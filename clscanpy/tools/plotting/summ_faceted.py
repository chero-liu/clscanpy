...
from clscanpy.tools.utils import save_figure
...
def summ_faceted_plot(
    ...
    if groupby_all == groupby:
        ...
        return _plot_single_level(
            adata, groupby, pointsize, color, palette, save, show, outdir, method
        )
    ...
    if layout_mode == "group_level":
        ...
        if max_label_length > 40:
            legend_x = (
                0.95 * (width / (subplot_size * ncols)) + 0.2 - (nrows - 1) * 0.01
            )
        print(legend_x)
    all_x = []
    all_y = []
...
def _plot_single_level(
    ...
    with rc_context({"figure.figsize": (width, height)}):
        ...
        center_y = (np.min(all_y) + np.max(all_y)) / 2
        x_range = (
            center_x - max_span_with_buffer / 2,
            center_x + max_span_with_buffer / 2,
        )
        y_range = (
            center_y - max_span_with_buffer / 2,
            center_y + max_span_with_buffer / 2,
        )
