...
class MultiSpeciesSplitter:
    ...
    def __init__(
        self, input_dir: str, mix_species: str, gene_prefix_clean: bool = True
    ):
        """
        Parameters
        ----------
    ...
    def _read_gem_classification(self) -> pd.DataFrame:
        ...
        gem_path = os.path.join(
            os.path.dirname(self.input_dir), "analysis", "gem_classification.csv"
        )
        if not os.path.exists(gem_path):
            raise FileNotFoundError(
                f"Cannot find gem_classification.csv at: {gem_path}"
            )

        data = pd.read_csv(gem_path)
        if "barcode" not in data.columns or "call" not in data.columns:
            raise ValueError(
                f"Missing required columns in {gem_path}: {list(data.columns)}"
            )
    ...
    def _filter_barcodes(self, key_data: pd.DataFrame) -> List[str]:
        ...
        self.logger.info(
            f"Filtered {len(filtered)} barcodes for species '{self.mix_species}'"
        )
        return filtered
    ...
    def _process_adata(self, adata: sc.AnnData) -> sc.AnnData:
        ...
        self.logger.info(
            f"Found {len(genes_of_interest)} genes with prefix '{self.mix_species}'"
        )
    ...
    def split(self, adata: sc.AnnData) -> sc.AnnData:
        ...
        self.logger.info(
            f"Starting split for species '{self.mix_species}' in sample '{self.sample_name}'"
        )
        ...
        self.logger.info(
            f"Final AnnData shape: {sub_adata.n_obs} cells × {sub_adata.n_vars} genes"
        )
...
def reset_barcode(
    ...
    barcode_pool_file: Path = Path(
        "/data/software/cellranger/cellranger-7.0.1/lib/python/cellranger/barcodes/3M-february-2018.txt.gz"
    ),
) -> List[str]:
...
def read(
    ...
    barcode_pool_file: Union[str, Path] = Path(
        "/data/software/cellranger/cellranger-7.0.1/lib/python/cellranger/barcodes/3M-february-2018.txt.gz"
    ),
):
        ...
        try:
            ...
            for col, levels in fact_dict.items():
                if col in metadata_py.columns:
                    ...
                    levels = (
                        list(levels)
                        if isinstance(levels, robjects.vectors.ListVector)
                        else levels
                    )
                    # 转换为有序分类
                    metadata_py[col] = pd.Categorical(
                        metadata_py[col], categories=levels, ordered=True
                    )
