...
def visualize_results(
    ...
    ## 绘制预测细胞类型图
    logger.info(
        f"绘制预测细胞类型({cell_type_key})图: {outdir}/{reduction}_{cell_type_key}.pdf/png"
    )
    ...
    if cell_type_key in adata.obs.columns:
        ...
        adata.obs[cell_type_key] = pd.Categorical(
            adata.obs[cell_type_key], categories=unique_types
        )
        ...
        fig = sc.pl.umap(
            adata, color=cell_type_key, palette=cell_type_colors, show=False
        )
    else:
        fig = sc.pl.umap(adata, color=cell_type_key, show=False)

    plt.savefig(
        f"{outdir}/{reduction}_{cell_type_key}.pdf", bbox_inches="tight", dpi=300
    )
    plt.savefig(
        f"{outdir}/{reduction}_{cell_type_key}.png", bbox_inches="tight", dpi=300
    )
    plt.close()
    ...
    if cluster_key:
        ...
        cluster_celltype = pd.crosstab(
            adata.obs[cluster_key], adata.obs["predicted_labels"]
        )

        # 计算每个簇中各细胞类型的比例
        cluster_celltype_pct = (
            cluster_celltype.div(cluster_celltype.sum(axis=1), axis=0) * 100
        )
...
@click.command()
@click.option("--input", required=True, help="输入文件路径，支持h5ad,h5seurat格式")
@click.option("--model-path", required=True, help="Celltypist模型路径，pkl格式")
@click.option(
    "--output-path",
    required=True,
...
@click.option(
    ...
    type=float,
    show_default=True,
    help="majority voting时，簇内主要类型的最小占比阈值，默认0",
)
@click.option(
    "--prefix", default="", show_default=True, type=str, help="输出文件的前缀,默认空"
)
@click.option(
    "--xlsx",
    default=False,
    show_default=True,
    help="是否将结果合并为Excel文件,默认False",
)
@click.option(
    "--plot-results",
    default=False,
    show_default=True,
    help="是否生成可视化结果,默认False",
)
@click.option(
    "--update-models", default=False, show_default=True, help="是否更新Celltypist模型"
)
@click.option(
    "--show-models", default=False, show_default=True, help="是否显示所有Celltypist模型"
)
@click.option(
    "--reduction",
    default="X_umap",
    show_default=True,
    help="UMAP降维方法，默认Xumap,用于可视化结果",
)
def main(
    input,
    model_path,
