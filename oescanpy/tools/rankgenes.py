...
from oescanpy.tools.markergenes.rank_genes_groups import rank_genes_groups
import scanpy as sc
...
class RankGenes:
    ...
    def run_RankGeneSpecified(
        ...
        LOGGER.info(f"Performing rank genes between {'_'.join(group1)} vs {'_'.join(group2)}...")
...
def get_opts_rankgenes(parser, sub_program=False):
    parser.add_argument('-i', "--input", type=str, default=None, help="Input file (AnnData object in .h5ad format)", required=True)
    parser.add_argument('-o', "--outdir", type=str, default=None, help="Output directory for results", required=True)
    parser.add_argument(
        "--sampleid",
        type=str,
        default=None,
