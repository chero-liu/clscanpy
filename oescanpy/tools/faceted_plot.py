...
from oescanpy.tools.plotting.summ_faceted import summ_faceted_plot,_plot_single_level
from oescanpy.tools.plotting.summ_contrast import summ_contrast_plot
    ...
    def run(self):
        ...
        for groupfactor in self.groupby:
            for facet_col in self.facetby:
                summ_faceted_plot(self.adata,groupby = facet_col,color = groupfactor,method=self.method,outdir=self.outdir,palette=self.palette)
                summ_contrast_plot(self.adata,method=self.method,color=facet_col,outdir=self.outdir,palette=self.palette)
    def __enter__(self):
        return self
...
def get_opts_faceted_plot(parser, sub_program=True):
    ...
    parser.add_argument('--groupby', type=str,default = None, help='分组变量',required=True)
    parser.add_argument('--facetby', type=str,default = None, help='分面分组变量',required=True)
