...
import pandas as pd
import numpy as np
from oescanpy.tools.plotting.violin import plot_qc_violin
from oescanpy.tools.color.utils import get_color_order
from oescanpy.tools.color.colors import add_color_to_dataframe
from oescanpy.tools.preprocessing.doublet import findDoublet
...
from oescanpy.tools.read.load import standardize_colors
from oescanpy.tools.preprocessing.basicpreprocessing import (
    _qc,
    _filter,
    _mtfilter,
    find_outliers,
    ...
    def run(self):
        ...
        if self.info.endswith(".csv"):
            ...
            for index, line in self.info.iterrows():
                ...
                if getattr(line, "mix_species", None) and line.mix_species.lower() != "no":
                    try:
                        splitter = MultiSpeciesSplitter(
                            input_dir=line.path,
                            mix_species=line.mix_species
                        )
                        adata = splitter.split(adata)
                        LOGGER.info(f"Successfully split species '{line.mix_species}' for {line.path}")
                    except FileNotFoundError as e:
                        LOGGER.warning(f"Missing required files for {line.path}: {e}")
                    except ValueError as e:
                        LOGGER.warning(f"Invalid data format for {line.path}: {e}")
                    except Exception as e:
                        LOGGER.exception(f"[Error] Unexpected error while splitting {line.path}: {e}")
                else:
                    LOGGER.debug(f"mix_species not specified or set to 'no' for {line.path}")
