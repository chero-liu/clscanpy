...
def transform_cluster_labels(adata: AnnData, cluster_method: str):
    ...
    adata.obs["clusters"] = adata.obs[cluster_method]

    # Convert cluster labels to numeric and find the maximum
    max_num = max(pd.to_numeric(adata.obs["clusters"]))

    # Rename categories in "clusters" to consecutive integers starting from 1
    adata.rename_categories("clusters", list(map(str, list(range(1, max_num + 2)))))
