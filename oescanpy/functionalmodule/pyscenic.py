import os
from oescanpy.tools.utils import s_common, Step, check_mkdir, check_adata
import unittest
import oescanpy as oep
from oescanpy.tools.preprocessing.basicpreprocessing import filter_genes_for_pyscenic
from oescanpy.script.pyscenic.utils import Analysis, Visualize
...
class pySCENIC(Step):
    ...
    def run(self):
        ...
        subprocess.run(['mv', f"{self.outdir}/data/regulons.xls", f"{self.outdir}/result/regulons.xls"], check=True)
        subprocess.run(['mv', f"{self.outdir}/data/TF_TargetGenes.xls", f"{self.outdir}/result/TF_TargetGenes.xls"], check=True)
